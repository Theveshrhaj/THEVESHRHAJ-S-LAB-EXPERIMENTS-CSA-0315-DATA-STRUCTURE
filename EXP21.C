...
int visited[MAX];
void enqueue(int v){ if(rear==MAX-1) return; if(front==-1) front=0; queue[++rear]=v; }
...
int main(){
    ...
    printf("BFS: ");
    while(front<=rear){
        int v=dequeue(); printf("%d ",v);
        for(int i=0;i<n;i++) if(adj[v][i]&&!visited[i]){ enqueue(i); visited[i]=1; }
    }
}